...
export default defineConfig(({ command, mode }) => {
  ...
  const jsonFolderPath = resolve(
    __dirname,
    `./src/apps/build-variables/${variation}.json`,
  );
  let build_variables = {};
  ...
  return {
    ...
    server: {
      // Project default (see cmd/main.go's PORT default for the backend,
      // 5001) - strictPort so a stray process squatting on 5002 fails
      // loudly instead of silently starting the UI on a different port.
      port: 5002,
      strictPort: true,
    },
    define: {
