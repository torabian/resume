...
export function AuthenticationProvider<TExtra = unknown>({
  ...
  /** Redirects to self-service's signout page; see `AuthenticationContextValue.signoutRemotely`. */
  const signoutRemotely = (returnUrl: string = window.location.href) => {
    window.location.href = buildSelfServiceSignoutUrl(
      selfServiceUrl,
      returnParam,
      returnUrl,
    );
