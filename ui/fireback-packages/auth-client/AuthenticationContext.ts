...
export interface AuthenticationContextValue<TExtra = unknown> {
  ...
  signout: () => void;
  /**
   * Sends the browser to self-service's own `/signout` page instead of
   * clearing the session locally - `returnUrl` (defaults to the current
   * page) is where self-service sends the user back once it's done.
   *
   * Unlike `signout`, this doesn't touch this app's session/storage at all
   * itself, which is the point: an app that only imports this lib as a
   * dependency (rather than being fireback's own self-service app) has no
   * business independently deciding what "signed out" means server-side -
   * self-service's `/signout` page is the one place that both invalidates
   * the passport with the backend and knows how to clear the session it
   * itself manages, and every importer should go through it rather than
   * reimplement that.
   */
  signoutRemotely: (returnUrl?: string) => void;
  /**
