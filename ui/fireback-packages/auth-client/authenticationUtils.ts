...
/**
 * Builds a self-service URL rooted at `selfServiceUrl` (e.g. "/selfservice",
 * as apps configure `AuthenticationProvider` with - a relative path, not a
 * full URL), with `path` appended and a `returnParam` query param set to
 * `returnUrl` so self-service knows where to send the user back once it's
 * done. `window.location.origin` is passed as the base so a relative
 * `selfServiceUrl` resolves correctly - `new URL()` throws on a bare relative
 * string with no base at all.
 */
function buildSelfServiceUrl(
  ...
  const url = new URL(selfServiceUrl.replace(/\/$/, "") + path, base);
...
export function buildSelfServiceLoginUrl(
  ...
  return buildSelfServiceUrl(selfServiceUrl, "", returnParam, returnUrl);
}

/**
 * Builds the URL `signoutRemotely()` sends the browser to: self-service's own
 * `/signout` page (SelfServiceRoutes.tsx), which does the actual clearing -
 * see `AuthenticationContextValue.signoutRemotely`'s doc comment for why that
 * has to happen there rather than locally. `returnParam` is set to
 * `returnUrl` so self-service knows where to send the user back once it's
 * done.
 */
export function buildSelfServiceSignoutUrl(
  selfServiceUrl: string,
  returnParam: string,
  returnUrl: string,
): string {
  return buildSelfServiceUrl(
    selfServiceUrl,
    "/signout",
    returnParam,
    returnUrl,
