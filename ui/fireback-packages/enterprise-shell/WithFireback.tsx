...
const WithFetchX = ({
  ...
  // Bug fix: defaultHeaders never set "accept-language" at all, so every request just
  // got whatever Accept-Language the browser sends by default (the OS/browser locale
  // list, e.g. "en-GB,en-US;q=0.9,...") - completely unrelated to the locale the user
  // actually picked in the app (localeStore.ts's setLocale/getLocale, persisted across
  // reloads). Every server-side translated error (see resolve.AuthResultDto.Language's
  // own doc comment) was silently keying off the wrong language as a result. Reads
  // straight from localeStore.ts's getLocale() (localStorage, kept in sync by
  // setLocale() whenever the user switches language) rather than the reactive
  // useLocale() hook - this component sits above the router (WithFireback wraps
  // AuthenticationProvider/FetchxProvider, mounted before any Route), and useLocale()
  // depends on router context that isn't available up here yet.
