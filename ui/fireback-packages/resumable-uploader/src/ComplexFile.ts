...
export class ComplexFile implements ComplexFileData {
  ...
  constructor(value?: ComplexFileInput | null) {
    // A file field is routinely absent (no upload yet) or explicitly
    // nulled out server-side (e.g. a locked/preview record withholding
    // protected content) - every emi-generated Dto setter that wraps a
    // non-ComplexFile value unconditionally does `new ComplexFile(value)`
    // with no null guard of its own (mirroring how every other sub-object
    // field's generated setter tolerates null/undefined by constructing
    // an empty instance rather than throwing), so this constructor has to
    // be the one place that absorbs it - reading `value.id` below on a
    // bare null/undefined would otherwise throw.
