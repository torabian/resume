...
export const RoleEntityManager = ({ data }: DtoEntity<RoleDto>) => {
  ...
  return (
    <CommonEntityManager
      ...
      // The seeded "root" role is a system fixture (RepairTheWorkspaces/
      // CreateRootRoleInWorkspace, WorkspaceActions.go) - RoleUpdateAction rejects
      // modifying it outright, so the form is locked read-only here too rather than
      // letting an admin fill it out only to have Save fail.
      disabled={uniqueId === "root"}
      data={data}
    />
  );
};
