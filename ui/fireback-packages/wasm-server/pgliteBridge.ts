...
export interface WasmQueryResult {
  // Positional, not keyed by column name - see the rowMode: "array" comment
  // on the db.query() call below for why.
  rows: unknown[][];
...
export async function installPgliteBridge(
  ...
  window.queryDatabase = async (query: string, args: unknown) => {
    try {
      ...
      // rowMode: "array" gets rows back as plain positional value arrays
      // instead of pglite's default `{ [columnName]: value }` object. That
      // default breaks the moment two result columns share a name (e.g. two
      // unaliased expressions - Postgres names both "?column?", such as
      // gorm's postgres migrator's introspection query, which has both
      // `c.is_nullable = 'YES'` and `8 * typlen` unaliased): the object form
      // can only keep one value per key, so the second column's value
      // silently clobbers the first's before it ever reaches Go. Positional
      // rows can't collide like that - wasmpgdriver's wasmRows.Next reads
      // dest[i] from row[i] (see modules/fireback/wasmpgdriver/Driver.go),
      // matching this array order against the (always-positional, so never
      // ambiguous) `fields` array below.
      const res = await db.query(query, params, { rowMode: "array" });

      return JSON.stringify({
        rows: (res.rows as unknown[][] | undefined) ?? [],
        fields: res.fields ?? [],
