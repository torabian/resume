...
/**
 * Action to communicate with the action walletProductGet
 */
export type WalletProductGetActionOptions = {
  queryKey?: unknown[];
  params: WalletProductGetActionPathParameter;
  qs?: URLSearchParams;
};
export type WalletProductGetActionQueryOptions = Omit<
  UseQueryOptions<unknown, unknown, GResponse<WalletProductDto>, unknown[]>,
  "queryKey"
> &
  WalletProductGetActionOptions &
  Partial<{
...
export const useWalletProductGetActionQuery = (
  options: WalletProductGetActionQueryOptions,
) => {
  ...
  const fn = () => {
    ...
    return WalletProductGetAction.Fetch(
      options.params,
      {
  ...
  const result = useQuery({
    queryKey: [WalletProductGetAction.NewUrl(options.params, options?.qs)],
    queryFn: fn,
...
export type WalletProductGetActionMutationOptions = Omit<
...
export const useWalletProductGetAction = (
  options: WalletProductGetActionMutationOptions,
) => {
  const globalCtx = useFetchxContext();
  const ctx = options?.ctx ?? globalCtx ?? undefined;
  const [isCompleted, setCompleteState] = useState(false);
...
/**
 * Path parameters for WalletProductGetAction
 */
export type WalletProductGetActionPathParameter = {
  uniqueId: string;
};
/**
 * WalletProductGetAction
 */
export class WalletProductGetAction {
  //
  static URL = "/walletProduct/:uniqueId";
  static NewUrl = (
    params: WalletProductGetActionPathParameter,
    qs?: URLSearchParams,
  ) => buildUrl(WalletProductGetAction.URL, params, qs);
  static Method = "GET";
  static Fetch$ = async (
    params: WalletProductGetActionPathParameter,
    qs?: URLSearchParams,
    ...
    return fetchx<GResponse<WalletProductDto>, unknown, unknown>(
      overrideUrl ?? WalletProductGetAction.NewUrl(params, qs),
      {
        method: WalletProductGetAction.Method,
        ...(init || {}),
      },
      ctx,
    );
  ...
  static Fetch = async (
    ...
    const res = await WalletProductGetAction.Fetch$(
      params,
      qs,
      ctx,
      init,
  ...
  static Definition = {
    name: "walletProductGet",
    cliName: "get",
    cliShort: "g",
    url: "/walletProduct/:uniqueId string",
    method: "get",
    description: 'Looks up a single "walletProduct" row by uniqueId.',
    out: {
      envelope: "GResponse",
