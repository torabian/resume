...
export class AdminCreateWalletAction {
  ...
  static Definition = {
    ...
    method: "post",
    description:
      "Root-only: creates a wallet on behalf of any user or workspace, not just the caller - the admin/support counterpart to walletpublic's createWallet (which only ever creates for the caller themselves). Enforces the same walletConfig.maxWalletsPer{User,Workspace} [PerCurrency] limits and active-currency check as the owner-facing path (see wallet.CheckWalletLimit in WalletLimit.go, shared by both). Rejects if the target userId/workspaceId doesn't exist. ownerType \"workspaceUser\" requires both userId and workspaceId - unlike walletpublic's createWallet, membership isn't checked here either.",
    in: {
      fields: [
        {
          name: "ownerType",
          description:
            'Whether the new wallet belongs to a user, a workspace, or one specific user scoped to one specific workspace ("workspaceUser" - see wallet.ownerType).',
          type: "string",
          tags: {
            validate: "required,oneof=user workspace workspaceUser",
          },
        },
        {
          name: "userId",
          description:
            'Unique id of the target user. Required when ownerType is "user" or "workspaceUser" - must be an existing user.',
          type: "string?",
          tags: {
        ...
        },
        {
          name: "workspaceId",
          description:
            'Unique id of the target workspace. Required when ownerType is "workspace" or "workspaceUser" - must be an existing workspace. Unlike walletpublic\'s createWallet, the caller does not need to be a member of it.',
          type: "string?",
          tags: {
