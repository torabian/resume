...
export class WalletTransactionOptionalDto {
   ...
   * @type {string}
   **/
  #balanceAfter?: string | null | undefined = undefined;
  /**
   * Snapshot of the wallet's balance immediately after this entry was applied, as a minor-units string. Kept for audit/debugging even though it's derivable by replaying the ledger.
   * @returns {string}
   **/
  ...
  /**
   * What kind of event produced this ledger entry. "redeem" is a prepaid gift card being redeemed (see the prepaid entity and walletpublic's redeemPrepaid action). "treasury_funding" is a treasury's linked wallet being capitalized (see the treasury entity and the fundTreasury action).
   * @type {string}
   **/
   ...
   * @type {string}
   **/
  #referenceType?: string | null | undefined = undefined;
  /**
   * Free-form identifier of the calling module/feature that caused this entry, e.g. "course-purchase" - lets other nima modules tag their own purchases without editing this module's yaml.
   * @returns {string}
   **/
   ...
   * @type {string}
   **/
  #createdBy?: string | null | undefined = undefined;
  /**
   * Unique id of the user who triggered this entry, or a fixed marker such as "system" or the gateway code, for entries with no human actor.
   * @returns {string}
   **/
  ...
  set createdAt(value: XDate) {
    if (value instanceof XDate) {
      ...
      this.#createdAt = new XDate(value);
    }
  }
