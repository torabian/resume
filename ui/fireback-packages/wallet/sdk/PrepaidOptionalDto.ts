...
/**
 * The base class definition for prepaidOptionalDto
 **/
export class PrepaidOptionalDto {
  /**
   *
   * @type {string}
   **/
   ...
   * @type {string}
   **/
  #redeemKey?: string | null | undefined = undefined;
  /**
   * The secret code checked/redeemed via walletpublic's checkPrepaid/ redeemPrepaid actions - whoever holds this key can redeem the card once. Auto-generated (fireback.GenerateSecureToken) by PrepaidCreateAction when left empty on create; unique across every prepaid card. Never regenerated afterwards - editing it via the regular update action orphans whatever it was originally issued/shared as, so root shouldn't, even though nothing in this schema prevents it.
   * @returns {string}
   **/
   ...
   * @type {string}
   **/
  set redeemKey(value: string | null | undefined) {
    const correctType =
      typeof value === "string" || value === undefined || value === null;
    this.#redeemKey = correctType ? value : String(value);
  }
  setRedeemKey(value: string | null | undefined) {
   ...
   * @type {boolean}
   **/
  #isExchangeable?: boolean | null | undefined = false;
  /**
   * Whether this card is conceptually allowed to be redeemed into a wallet of a different currency than currency above. Informational only for now - surfaced in checkPrepaid's response, but redeemPrepaid always requires an exact currency match regardless of this flag, since no exchange-rate source exists anywhere in this codebase to safely convert an amount. Wire this up for real once one does.
   * @returns {boolean}
   **/
  ...
  applyFromObject(data = {}) {
    const d = data as Partial<PrepaidOptionalDto>;
  ...
  /**
   * Creates an instance of PrepaidOptionalDto, and possibleDtoObject
   * needs to satisfy the type requirement fully, otherwise typescript compile would
   * be complaining.
   **/
  static from(possibleDtoObject: PrepaidOptionalDtoType) {
    return new PrepaidOptionalDto(possibleDtoObject);
  }
  /**
   * Creates an instance of PrepaidOptionalDto, and partialDtoObject
   * needs to satisfy the type, but partially, and rest of the content would
   * be constructed according to data types and nullability.
   **/
  static with(partialDtoObject: PartialDeep<PrepaidOptionalDtoType>) {
    return new PrepaidOptionalDto(partialDtoObject);
  }
  copyWith(
    partial: PartialDeep<PrepaidOptionalDtoType>,
  ): InstanceType<typeof PrepaidOptionalDto> {
    return new PrepaidOptionalDto({ ...this.toJSON(), ...partial });
  }
  clone(): InstanceType<typeof PrepaidOptionalDto> {
    return new PrepaidOptionalDto(this.toJSON());
  }
}
export abstract class PrepaidOptionalDtoFactory {
  abstract create(data: unknown): PrepaidOptionalDto;
}
/**
 * The base type definition for prepaidOptionalDto
 **/
export type PrepaidOptionalDtoType = {
  /**
   *
   * @type {string}
   **/
...
export namespace PrepaidOptionalDtoType {}
