...
export class CreateWalletAction {
  ...
  static Definition = {
    ...
    method: "post",
    description:
      'Creates a new wallet for the caller (ownerType "user"), for a workspace the caller belongs to (ownerType "workspace"), or for the caller scoped to one specific workspace they belong to (ownerType "workspaceUser" - see wallet.ownerType; used e.g. to have somewhere to redeem a workspace-restricted prepaid gift card into), in the given currency, with a starting balance of "0". Rejected if it would exceed walletConfig\'s maxWalletsPer{User,Workspace}[PerCurrency] limits, or if currency has no active walletCurrency.',
    in: {
      fields: [
        {
          name: "ownerType",
          description:
            'Whether this wallet belongs to the caller directly, to a workspace, or to the caller scoped to one specific workspace ("workspaceUser").',
          type: "string",
          tags: {
        ...
        },
        {
          name: "workspaceId",
          description:
            'Unique id of the owning workspace. Required when ownerType is "workspace" or "workspaceUser" - the caller must belong to this workspace either way.',
          type: "string?",
          tags: {
