...
import {
  buildJsonLogic,
  isEmptyJsonLogic,
} from "../components/common-data-table/jsonLogic";
...
// Exported so callers that need to tell "did the user actually filter
// anything" apart from "these are just the pagination/sort bookkeeping
// keys udf.filters always carries" - see CommonListManager.tsx's own
// hasActiveFilters - can reuse the exact same list instead of maintaining a
// second copy that could drift out of sync with what buildUdfBrowseQs below
// treats as a real column filter.
