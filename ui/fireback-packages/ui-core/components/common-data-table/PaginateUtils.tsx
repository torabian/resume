...
/**
 * Renders a raw cell value for a column with no explicit getCellValue (see
 * castColumns' renderCell below) - safe against a value that's a real
 * complexes class instance (TString, TMoney, XDate, XDateTime, or PlainTime
 * - a Date subclass) rather than the plain primitive React can render
 * directly. Handing one of those straight to JSX throws "Objects are not
 * valid as a React child" - this is the same class of bug a column's own
 * getCellValue can just as easily have (see e.g. UserColumns.tsx's own bug-
 * fix comment on its birthDate column), just centralized here for every
 * column that never wrote one at all, auto-generated createdAt/updatedAt
 * columns included.
 *
 * Every one of those classes' own toString()/valueOf() already produces a
 * reasonable display value (TString/TMoney fall back to a default locale/
 * currency rather than the viewer's own - a real column wanting that should
 * still write its own getCellValue calling getTStringValue/.get(locale)
 * directly), so String(value) is a safe, generic fallback for any object,
 * not just these specific classes - wrapped in try/catch as a last resort
 * against anything with a throwing toString() of its own.
 */
export function formatCellFallbackValue(value: unknown): string {
  if (value == null) {
    return "";
  }
  ...
  return columns.map((col) => {
    ...
    return {
      ...
      renderCell: ({ column, row }) => {
        ...
        return (
          <span>{formatCellFallbackValue(get(row, column.key as string))}</span>
        );
      },
