...
export const UserPassportList = ({ userId }: { userId: string }) => {
  // PassportBrowseActionQuery filters via a jsonlogic `filter` expression now (see
  // emigorm.ApplyQueryFilter) - the old raw-SQL-fragment `query` param this used to send
  // is a field PassportBrowseActionQuery no longer even has, so it was silently ignored,
  // making this list return *every* passport in the system unfiltered (the user-detail
  // screen's "Set password"/"Disable 2FA"/"Send reset email" buttons could then act on
  // the wrong passport row - e.g. root's own, if it happened to render first).
  const { data, refetch } = usePassportBrowseActionQuery({
    qs: userId
      ? new URLSearchParams({
          filter: JSON.stringify({ "==": [{ var: "user_id" }, userId] }),
        })
      : undefined,
  });
