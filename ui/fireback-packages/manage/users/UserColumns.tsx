...
export const columns = (
...

  {
    ...
    title: s.birthDate,
    width: 140,
    // Bug fix: e.birthDate is a real complexes.XDate instance (was
    // effectively unreachable before @fireback/complexes actually exported
    // XDate - see Wallet.emi.yml/Abac.emi.yml's complexes: block - the
    // setter threw "XDate is not defined" the instant any UserDto with a
    // birthDate was constructed). Handing that instance straight to JSX as
    // a raw child throws "Objects are not valid as a React child" instead
    // of rendering text - toDateString() is XDate's own "give me the plain
    // YYYY-MM-DD string" accessor.
    getCellValue: (e: UserDto) => <>{e?.birthDate?.toDateString() ?? ""}</>,
    filterType: "date",
    filterable: true,
    sortable: true,
  },
