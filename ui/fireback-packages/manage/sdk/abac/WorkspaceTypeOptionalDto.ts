import { PlainTime, TString } from "@fireback/complexes";
...
export class WorkspaceTypeOptionalDto {
  ...
  /**
   * Display title, as a locale -> text map (e.g. {"en": "Customer", "fa": "مشتری"}) - see complexes.TString.
   * @type {TString}
   **/
  #title!: TString;
  /**
   ...
   * @type {TString}
   **/
  set title(value: TString) {
    if (value instanceof TString) {
      this.#title = value;
    } else {
      this.#title = new TString(value);
    }
  }
  setTitle(value: TString) {
    this.title = value;
    return this;
  }
  /**
   ...
   * @type {string}
   **/
  #slug?: string | null | undefined = undefined;
  /**
   * Unique, URL-safe identifier for this workspace type. Must start with "/", contain only lowercase a-z and dashes after that, and be at most 50 characters long. Format is enforced in ValidateTheWorkspaceTypeEntity (WorkspaceTypeActions.go); gorm:"unique" here is the DB-level backstop for the uniqueness half of that.
   * @returns {string}
   **/
