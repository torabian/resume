...
/**
 * Action to communicate with the action skillGet
 */
export type SkillGetActionOptions = {
  queryKey?: unknown[];
  params: SkillGetActionPathParameter;
  qs?: URLSearchParams;
};
export type SkillGetActionQueryOptions = Omit<
  UseQueryOptions<unknown, unknown, GResponse<SkillDto>, unknown[]>,
  "queryKey"
> &
  SkillGetActionOptions &
  Partial<{
...
export const useSkillGetActionQuery = (options: SkillGetActionQueryOptions) => {
  ...
  const result = useQuery({
    queryKey: [SkillGetAction.NewUrl(options.params, options?.qs)],
    queryFn: fn,
...
export type SkillGetActionMutationOptions = Omit<
...
export const useSkillGetAction = (options: SkillGetActionMutationOptions) => {
  const globalCtx = useFetchxContext();
  const ctx = options?.ctx ?? globalCtx ?? undefined;
  const [isCompleted, setCompleteState] = useState(false);
  const [response, setResponse] = useState<TypedResponse<unknown>>();
 ...
 */
export type SkillGetActionPathParameter = {
  uniqueId: string;
};
/**
 * SkillGetAction
 */
export class SkillGetAction {
  //
  static URL = "/skill/:uniqueId";
  static NewUrl = (params: SkillGetActionPathParameter, qs?: URLSearchParams) =>
    buildUrl(SkillGetAction.URL, params, qs);
  static Method = "GET";
  static Fetch$ = async (
    params: SkillGetActionPathParameter,
    qs?: URLSearchParams,
    ...
    return fetchx<GResponse<SkillDto>, unknown, unknown>(
      overrideUrl ?? SkillGetAction.NewUrl(params, qs),
      {
        method: SkillGetAction.Method,
        ...(init || {}),
      },
      ctx,
    );
  ...
  static Fetch = async (
    ...
    const res = await SkillGetAction.Fetch$(params, qs, ctx, init, overrideUrl);
