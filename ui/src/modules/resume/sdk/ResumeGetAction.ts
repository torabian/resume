...
/**
 * Action to communicate with the action resumeGet
 */
export type ResumeGetActionOptions = {
  queryKey?: unknown[];
  params: ResumeGetActionPathParameter;
  qs?: URLSearchParams;
};
export type ResumeGetActionQueryOptions = Omit<
  UseQueryOptions<unknown, unknown, GResponse<ResumeDto>, unknown[]>,
  "queryKey"
> &
  ResumeGetActionOptions &
  Partial<{
...
export const useResumeGetActionQuery = (
  options: ResumeGetActionQueryOptions,
) => {
  ...
  const result = useQuery({
    queryKey: [ResumeGetAction.NewUrl(options.params, options?.qs)],
    queryFn: fn,
...
export type ResumeGetActionMutationOptions = Omit<
...
export const useResumeGetAction = (options: ResumeGetActionMutationOptions) => {
  const globalCtx = useFetchxContext();
  const ctx = options?.ctx ?? globalCtx ?? undefined;
  const [isCompleted, setCompleteState] = useState(false);
  const [response, setResponse] = useState<TypedResponse<unknown>>();
 ...
 */
export type ResumeGetActionPathParameter = {
  uniqueId: string;
};
/**
 * ResumeGetAction
 */
export class ResumeGetAction {
  //
  static URL = "/resume/:uniqueId";
  static NewUrl = (
    params: ResumeGetActionPathParameter,
    qs?: URLSearchParams,
  ) => buildUrl(ResumeGetAction.URL, params, qs);
  static Method = "GET";
  static Fetch$ = async (
    params: ResumeGetActionPathParameter,
    qs?: URLSearchParams,
    ...
    return fetchx<GResponse<ResumeDto>, unknown, unknown>(
      overrideUrl ?? ResumeGetAction.NewUrl(params, qs),
      {
        method: ResumeGetAction.Method,
        ...(init || {}),
      },
      ctx,
    );
  ...
  static Fetch = async (
    ...
    const res = await ResumeGetAction.Fetch$(
      params,
      qs,
      ctx,
      init,
