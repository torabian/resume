...
/**
 * Action to communicate with the action languageGet
 */
export type LanguageGetActionOptions = {
  queryKey?: unknown[];
  params: LanguageGetActionPathParameter;
  qs?: URLSearchParams;
};
export type LanguageGetActionQueryOptions = Omit<
  UseQueryOptions<unknown, unknown, GResponse<LanguageDto>, unknown[]>,
  "queryKey"
> &
  LanguageGetActionOptions &
  Partial<{
...
export const useLanguageGetActionQuery = (
  options: LanguageGetActionQueryOptions,
) => {
  ...
  const fn = () => {
    ...
    return LanguageGetAction.Fetch(
      options.params,
      {
  ...
  const result = useQuery({
    queryKey: [LanguageGetAction.NewUrl(options.params, options?.qs)],
    queryFn: fn,
...
export type LanguageGetActionMutationOptions = Omit<
...
export const useLanguageGetAction = (
  options: LanguageGetActionMutationOptions,
) => {
  const globalCtx = useFetchxContext();
  const ctx = options?.ctx ?? globalCtx ?? undefined;
  const [isCompleted, setCompleteState] = useState(false);
 ...
 */
export type LanguageGetActionPathParameter = {
  uniqueId: string;
};
/**
 * LanguageGetAction
 */
export class LanguageGetAction {
  //
  static URL = "/language/:uniqueId";
  static NewUrl = (
    params: LanguageGetActionPathParameter,
    qs?: URLSearchParams,
  ) => buildUrl(LanguageGetAction.URL, params, qs);
  static Method = "GET";
  static Fetch$ = async (
    params: LanguageGetActionPathParameter,
    qs?: URLSearchParams,
    ...
    return fetchx<GResponse<LanguageDto>, unknown, unknown>(
      overrideUrl ?? LanguageGetAction.NewUrl(params, qs),
      {
        method: LanguageGetAction.Method,
        ...(init || {}),
      },
      ctx,
    );
  ...
  static Fetch = async (
    ...
    const res = await LanguageGetAction.Fetch$(
      params,
      qs,
      ctx,
      init,
