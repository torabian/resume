...
/**
 * Action to communicate with the action companyGet
 */
export type CompanyGetActionOptions = {
  queryKey?: unknown[];
  params: CompanyGetActionPathParameter;
  qs?: URLSearchParams;
};
export type CompanyGetActionQueryOptions = Omit<
  UseQueryOptions<unknown, unknown, GResponse<CompanyDto>, unknown[]>,
  "queryKey"
> &
  CompanyGetActionOptions &
  Partial<{
...
export const useCompanyGetActionQuery = (
  options: CompanyGetActionQueryOptions,
) => {
  ...
  const result = useQuery({
    queryKey: [CompanyGetAction.NewUrl(options.params, options?.qs)],
    queryFn: fn,
...
export type CompanyGetActionMutationOptions = Omit<
...
export const useCompanyGetAction = (
  options: CompanyGetActionMutationOptions,
) => {
  const globalCtx = useFetchxContext();
  const ctx = options?.ctx ?? globalCtx ?? undefined;
  const [isCompleted, setCompleteState] = useState(false);
 ...
 */
export type CompanyGetActionPathParameter = {
  uniqueId: string;
};
/**
 * CompanyGetAction
...
export class CompanyGetAction {
  ...
  static NewUrl = (
    params: CompanyGetActionPathParameter,
    qs?: URLSearchParams,
  ) => buildUrl(CompanyGetAction.URL, params, qs);
  static Method = "GET";
  static Fetch$ = async (
    params: CompanyGetActionPathParameter,
    qs?: URLSearchParams,
    ...
    return fetchx<GResponse<CompanyDto>, unknown, unknown>(
      overrideUrl ?? CompanyGetAction.NewUrl(params, qs),
      {
        method: CompanyGetAction.Method,
        ...(init || {}),
      },
      ctx,
    );
  ...
  static Fetch = async (
    ...
    const res = await CompanyGetAction.Fetch$(
      params,
      qs,
      ctx,
      init,
