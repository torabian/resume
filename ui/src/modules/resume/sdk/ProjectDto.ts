...
export class ProjectDto {
   ...
   * @type {ProjectDto.Descriptions}
   **/
  #descriptions: MArray<InstanceType<typeof ProjectDto.Descriptions>> =
    MArray.of([]);
  /**
   * The project description, based on the target profile. So you can emphesize more on backend or front-end part of the project.
   * @returns {ProjectDto.Descriptions}
   **/
  get descriptions() {
  ...
  set descriptions(
    value:
      | MArray<InstanceType<typeof ProjectDto.Descriptions>>
      | InstanceType<typeof ProjectDto.Descriptions>[],
  ) {
    // When the passed value is already an array, we check if we need to
    // cast the inner items into class instance.
    if (Array.isArray(value)) {
      if (value.length > 0 && value[0] instanceof ProjectDto.Descriptions) {
        this.#descriptions = MArray.of(value);
      } else {
        this.#descriptions = MArray.of(
          value.map((item) => new ProjectDto.Descriptions(item)),
        );
      }
      return;
    }
  ...
  static Descriptions = class Descriptions {
    ...
    /**
     * Creates an instance of ProjectDto.Descriptions, and possibleDtoObject
     * needs to satisfy the type requirement fully, otherwise typescript compile would
     * be complaining.
     **/
    static from(possibleDtoObject: ProjectDtoType.DescriptionsType) {
      return new ProjectDto.Descriptions(possibleDtoObject);
    }
    /**
     * Creates an instance of ProjectDto.Descriptions, and partialDtoObject
     * needs to satisfy the type, but partially, and rest of the content would
     * be constructed according to data types and nullability.
     **/
    static with(
      partialDtoObject: PartialDeep<ProjectDtoType.DescriptionsType>,
    ) {
      return new ProjectDto.Descriptions(partialDtoObject);
    }
    copyWith(
      partial: PartialDeep<ProjectDtoType.DescriptionsType>,
    ): InstanceType<typeof ProjectDto.Descriptions> {
      return new ProjectDto.Descriptions({ ...this.toJSON(), ...partial });
    }
    clone(): InstanceType<typeof ProjectDto.Descriptions> {
      return new ProjectDto.Descriptions(this.toJSON());
    }
  };
  static JsonSchema = {
    type: "object",
  ...
  static get Fields() {
    return {
      ...
      get descriptions() {
        return withPrefix("descriptions[:i]", ProjectDto.Descriptions.Fields);
      },
      name: "name",
      role: "role",
      summary: "summary",
...
export type ProjectDtoType = {
  ...
  /**
   * The project description, based on the target profile. So you can emphesize more on backend or front-end part of the project.
   * @type {ProjectDtoType.DescriptionsType[]}
   **/
  descriptions: ProjectDtoType.DescriptionsType[];
  /**
   *
   * @type {string}
   **/
