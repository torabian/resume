...
export interface VirtualEntityManagerProps<T = any> {
  ...
  schema: RJSFSchema;
  uiSchema?: UiSchema;
  /** Custom rjsf field/widget components, for properties a plain JSON Schema
   * type can't express - e.g. a `complex` field like TString (a locale ->
   * value map, not a plain string - see @fireback/complexes/TString), which
   * needs its own widget rather than the default per-`type` one. Reference
   * a key here from `uiSchema`'s `"ui:field"`/`"ui:widget"` on that
   * property. */
  rjsfFields?: RegistryFieldsType;
  rjsfWidgets?: RegistryWidgetsType;
